...
# Create base storage folder and primary mount target with SGID
mkdir -p "$STORAGE_BASE"
chgrp "$STORAGE_GROUP" "$STORAGE_BASE" 2>/dev/null || true
chmod "$STORAGE_PERMS" "$STORAGE_BASE" 2>/dev/null || true
...
if [ -n "${MOUNT_BACKUP:-}" ]; then
    ...
    chmod "$STORAGE_PERMS" "$MOUNT_BACKUP" 2>/dev/null || true
fi
...
echo -e "\n${CLR_CYAN}[4/5] Generazione /etc/webdav/config.yaml (Root Base: ${CLR_WHITE}${STORAGE_BASE}${CLR_CYAN})...${CLR_RESET}"
mkdir -p /etc/webdav

export WEBDAV_USER WEBDAV_PASSWORD_HASH STORAGE_BASE WEBDAV_PORT
envsubst '$WEBDAV_USER $WEBDAV_PASSWORD_HASH $STORAGE_BASE $WEBDAV_PORT' < "$YAML_TEMPLATE_FILE" > /etc/webdav/config.yaml
...
echo -e "Il server WebDAV è pronto sulla porta ${CLR_WHITE}${WEBDAV_PORT}${CLR_RESET} esponendo l'intera radice storage '${CLR_WHITE}${STORAGE_BASE}${CLR_RESET}' (con tutti i volumi sottostanti)."
